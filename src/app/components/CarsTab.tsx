...
import { Car, CarFormDialog } from "./CarFormDialog";
...
export function CarsTab({ cars, onAddCar, onUpdateCar, onDeleteCar }: CarsTabProps) {
  ...
  const handleSave = (carData: Omit<Car, "id"> & { id?: string }) => {
    if (carData.id) {
      onUpdateCar(carData as Car);
    } else {
      onAddCar(carData);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <Button onClick={() => { setEditingCar(undefined); setIsDialogOpen(true); }}>
